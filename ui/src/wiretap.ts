...
export class WiretapComponent extends LitElement {
  ...
  private readonly _wiretapTrafficControlChannel: Channel;
  ...
  private _trafficControlChannelSubscription: Subscription;
  ...
  constructor() {
    ...
    if (wipeCache && wipeCache == "true") {
      localforage.clear().then(() => {
        console.log(
          "new version of wiretap detected, wiping application state."
        );
    ...
    this._httpTransactionStore = this._storeManager.createBag<HttpTransaction>(
      WiretapHttpTransactionStore
    );

    // create selected transaction store
    this._selectedTransactionStore =
      this._storeManager.createBag<HttpTransaction>(
        WiretapSelectedTransactionStore
      );
    ...
    // controls store
    this._controlsStore =
      this._storeManager.createBag<WiretapControls>(WiretapControlsStore);
    ...
    this._wiretapTrafficControlChannel = this._bus.createChannel(
      TrafficControlChannel
    );
    this._wiretapSpecChannel = this._bus.createChannel(SpecChannel);
    this._wiretapControlsChannel = this._bus.createChannel(
      WiretapControlsChannel
    );
    this._wiretapReportChannel = this._bus.createChannel(WiretapReportChannel);
    this._wiretapConfigChannel = this._bus.createChannel(
      WiretapConfigurationChannel
    );
    this._staticNotificationChannel =
      this._bus.createChannel(WiretapStaticChannel);

    // map local bus channels to broker destinations.
    this._bus.mapChannelToBrokerDestination(
      TopicPrefix + WiretapChannel,
      WiretapChannel
    );
    this._bus.mapChannelToBrokerDestination(
      QueuePrefix + SpecChannel,
      SpecChannel
    );
    this._bus.mapChannelToBrokerDestination(
      QueuePrefix + TrafficControlChannel,
      TrafficControlChannel
    );
    this._bus.mapChannelToBrokerDestination(
      QueuePrefix + WiretapControlsChannel,
      WiretapControlsChannel
    );
    this._bus.mapChannelToBrokerDestination(
      QueuePrefix + WiretapReportChannel,
      WiretapReportChannel
    );
    this._bus.mapChannelToBrokerDestination(
      QueuePrefix + WiretapConfigurationChannel,
      WiretapConfigurationChannel
    );
    this._bus.mapChannelToBrokerDestination(
      TopicPrefix + WiretapStaticChannel,
      WiretapStaticChannel
    );

    // handle incoming messages on different channels.
    this._transactionChannelSubscription = this._wiretapChannel.subscribe(
      this.wireTransactionHandler()
    );
    this._specChannelSubscription = this._wiretapSpecChannel.subscribe(
      this.specHandler()
    );
    this._trafficControlChannelSubscription =
      this._wiretapTrafficControlChannel.subscribe(
        this.trafficControlHandler()
      );

    this._configChannelSubscription = this._wiretapConfigChannel.subscribe(
      this.configHandler()
    );
    this._staticChannelSubscription = this._staticNotificationChannel.subscribe(
      this.staticHandler()
    );

    // load previous transactions from local storage.
    this.loadHistoryFromLocalStorage().then(
      (previousTransactions: Map<string, HttpTransaction>) => {
  ...
  firstUpdated() {
    ...
    const config = {
      brokerURL:
        protocol + this._wiretapHost + ":" + this._wiretapPort + "/ranch",
      heartbeatIncoming: 0,
      heartbeatOutgoing: 0,
      onConnect: () => {
        this.requestSpec();
  ...
  async loadHistoryFromLocalStorage(): Promise<Map<string, HttpTransaction>> {
    return localforage.getItem<Map<string, HttpTransaction>>(
      WiretapLocalStorage
    );
  }
  ...
  wireTransactionHandler(): BusCallback {
    return (msg: CommandResponse) => {
      ...
      const createTransaction = (): HttpTransaction => {
        ...
        constructedTransaction.httpRequest = Object.assign(
          new HttpRequest(),
          wiretapMessage?.httpRequest
        );
        constructedTransaction.id = wiretapMessage.id;
        constructedTransaction.requestValidation =
          wiretapMessage.requestValidation;
        ...
        if (linkCache) {
          linkCache.forEach(
            (value: Map<string, HttpTransactionBase[]>, key: string) => {
        ...
        if (
          wiretapMessage.requestValidation &&
          wiretapMessage.requestValidation.length > 0
        ) {
      ...
      if (existingTransaction && wiretapMessage.httpResponse) {
        ...
        if (
          ...
          wiretapMessage.responseValidation.length > 0
        ) {
          this.violatedTransactions++;
        }
        existingTransaction.httpResponse = Object.assign(
          new HttpResponse(),
          wiretapMessage?.httpResponse
        );
        existingTransaction.responseValidation =
          wiretapMessage.responseValidation;
  ...
  calcComplianceLevel(): void {
    ...
    if (this.violatedTransactions > 0) {
      this.complianceLevel =
        100 -
        parseFloat(
          ((this.violatedTransactions / this.requestCount) * 100).toFixed(2)
        );
    } else {
  ...
  render() {
    ...
    if (noSpec) {
      return html` <pb33f-header
          name="wiretap"
          url="https://pb33f.io/wiretap/?ref=wiretap"
        >
          <wiretap-header
            @wipeData=${this.wipeData}
            requests="${this.requestCount.toFixed(
              this._headerStatsDefaultPrecision
            )}"
            responses="${this.responseCount.toFixed(
              this._headerStatsDefaultPrecision
            )}"
            violations="${this.violationsCount.toFixed(
              this._headerStatsDefaultPrecision
            )}"
            violationsDelta="${this.violatedTransactions.toFixed(
              this._headerStatsDefaultPrecision
            )}"
            compliance="${this.complianceLevel.toFixed(
              this._complianceStatPrecision
            )}"
            noSpec
          >
          </wiretap-header>
