...
export class HttpTransactionViewComponent extends LitElement {
  ...
  constructor() {
    ...
    this._httpTransactionStore = GetBagManager().getBag<HttpTransaction>(
    ...
    if (value) {
      ...
      if (this._requestHeadersView && value.httpRequest) {
        ...
        if (value.httpRequest?.injectedHeaders) {
          this._injectedHeadersView.data = new Map(
            Object.entries(value.httpRequest?.injectedHeaders)
          );
        }
  ...
  render() {
    if (this._httpTransaction) {
      ...
      const requestViolations: TemplateResult = html` ${this._httpTransaction
        ?.requestValidation?.length > 0
        ? html`<h3>Request Violations</h3>`
        : html``}
      ...
      const responseViolations: TemplateResult = html` ${this._httpTransaction
        ?.responseValidation?.length > 0
        ? html`<h3>Response Violations</h3>`
        : html``}
      ...
      if (
        ...
        violations = html`Violations
          <sl-badge variant="danger" class="violation-badge"
            >${total}</sl-badge
          >`;
      }
      const noData: TemplateResult = html` <div class="empty-data ok">
      ...
      if (
        req.injectedHeaders?.size > 0 ||
        req.droppedHeaders?.length > 0 ||
        req.path != req.originalPath
      ) {
        originalTab = html` <sl-tab
          slot="nav"
          panel="request-origins"
          class="tab-secondary"
          >Mutations</sl-tab
        >`;
      } else {
        ...
        droppedHeaders = html`
          ...
          <ul>
            ${req.droppedHeaders.map((h) => {
              return html` <li class="dropped-header">${h}</li>`;
            })}
          </ul>
        `;
      }

      const tabGroup: TemplateResult = html` <sl-tab-group
        id="tabs"
        @sl-tab-show=${this.tabSelected}
      >
        <sl-tab slot="nav" panel="violations" id="violation-tab" class="tab"
          >${violations}</sl-tab
        >
        <sl-tab slot="nav" panel="request" class="tab">Request</sl-tab>
        <sl-tab slot="nav" panel="response" class="tab"
          >Response
          ${resp?.statusCode >= 400
            ? html` <sl-badge
                variant="${resp?.statusCode >= 400 && resp?.statusCode < 500
                  ? "warning"
                  : "danger"}"
            ...
            : null}</sl-tab
        >
        ${this._currentLinks?.length > 0
          ? html` <sl-tab slot="nav" panel="chain" class="tab">Chain</sl-tab>`
          : null}
        <sl-tab-panel name="violations" class="tab-panel">
          ${total <= 0 ? noData : null} ${requestViolations}
          ${this._httpTransaction?.requestValidation?.length > 0 &&
          this._httpTransaction?.responseValidation?.length > 0
            ? html` <hr />`
            : null}
        ...
        <sl-tab-panel name="request">
          <sl-tab-group class="secondary-tabs" placement="start">
            ...
            <sl-tab slot="nav" panel="request-headers" class="tab-secondary"
              >Headers</sl-tab
            >
            <sl-tab slot="nav" panel="request-cookies" class="tab-secondary"
              >Cookies</sl-tab
            >
            ${originalTab}
            ...
            <sl-tab-panel name="request-origins">
              ${req.injectedHeaders != null
                ? html`<h3>Injected headers</h3>
                    ${this._injectedHeadersView}
                    <hr />`
                : null}
              ${req.originalPath != req.path
                ? html`<h3>Destination details</h3>
                    ${this._originalDetailsView}
                    <hr />`
                : null}
              ${droppedHeaders}
            </sl-tab-panel>
        ...
        <sl-tab-panel name="response">
          <sl-tab-group class="secondary-tabs" placement="start">
            <sl-tab slot="nav" panel="response-body" class="tab-secondary"
              >Body</sl-tab
            >
            <sl-tab slot="nav" panel="response-code" class="tab-secondary">
              ...
              ${resp?.statusCode >= 400
                ...
                : null}</sl-tab
            >
            <sl-tab slot="nav" panel="response-headers" class="tab-secondary"
              >Headers</sl-tab
            >
            <sl-tab slot="nav" panel="response-cookies" class="tab-secondary"
              >Cookies</sl-tab
            >
            <sl-tab-panel name="response-code">
              <h2 class="${ExtractStatusStyleFromCode(resp)}">
                ${resp.statusCode}
              </h2>
              <h3>${ExtractHTTPCodeDefinition(resp)}</h3>
              <p class="response-code">${ExtractHTTPCodeDescription(resp)}</p>
            </sl-tab-panel>
      ...
      return html` <div class="empty-data engage">
  ...
  renderLinkMatch(
    ...
    linkMatch.siblings.forEach((sibling) => {
      const transaction = BuildLiveTransactionFromState(
        this._httpTransactionStore.get(sibling.id)
      );
      //if (transaction.id !== this._httpTransaction.id) {
      const siblingComponent = new HttpTransactionItemComponent(
        transaction,
        this._linkCache
      );
      ...
      if (
        ...
        this._chainTransactionView.httpTransaction?.id == transaction.id
      ) {
        siblingComponent.setActive();
      } else {
        siblingComponent.disable();
      }
      siblingComponent.addEventListener(
        HttpTransactionSelectedEvent,
        this.chainTransactionSelected.bind(this)
      );
      ...
      if (thisDiff != transaction.httpRequest.timestamp) {
        ...
        tsTotal =
          tsTotal +
          thisDiff +
          (transaction.httpResponse?.timestamp -
            transaction.httpRequest?.timestamp);
      }
      ...
      const timelineItem = html` <pb33f-timeline-item>
    ...
    return html` <div class="kv-overview">
        ...
        <div class="empty-data chain-time">
          ...
          <span class="total-time"
            ><span class="time-value">${tsTotalFormat}</span></span
          >
        </div>
      </div>
      <pb33f-timeline> ${timelineItems.reverse()} </pb33f-timeline>
      <div>${siblings.length <= 0 ? this.noOtherLinks() : null}</div>`;
  }

  noOtherLinks(): TemplateResult {
    return html` <div class="empty-data no-chain">
  ...
  renderChainTabPanel(): TemplateResult {
    const selectChain = () => {
      if (this._siblings?.length > 0) {
        return html` <div class="empty-data select-chain">
          <sl-icon name="link-45deg" class="binary-icon"></sl-icon>
          <br />
    ...
    if (!this.hideChain) {
      return html` <sl-tab-panel name="chain">
        <section class="chain-panel-divider">
          <div class="chain-container">
            ${this._currentLinks.map(
              (linkMatch) => html`${this.renderLinkMatch.bind(this)(linkMatch)}`
            )}
          </div>
